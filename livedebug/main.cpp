        ...
        while (window.isOpen())
        {
            ...
            while (window.pollEvent(event))
            {
                ...
                if (event.type == sf::Event::KeyPressed)
                {
                    ...
                    if (event.key.code == sf::Keyboard::A || event.key.code == sf::Keyboard::D ||
                        ...
                        ra += 5.0f*(event.key.code == sf::Keyboard::A ? -1.0f : 0.0f) + 5.0f*(event.key.code == sf::Keyboard::D ? 1.0f : 0.0f);
                        if (ra > 360.0f) ra -= 360.0f;
                        if (ra < 0.0f) ra += 360.0f;
                        de += 5.0f*(event.key.code == sf::Keyboard::W ? 1.0f : 0.0f) + 5.0f*(event.key.code == sf::Keyboard::S ? -1.0f : 0.0f);
                        ...
                        snprintf(buffer, sizeof(buffer), "sfml-tests/frame_%04zu.png", sprites.size() - 1);
            ...
            for (size_t i = 0; i < stars.size(); i++) {
                ...
                if (isMatched && haveCenter) {
                    ...
                    starText.setString("Star Name: " + std::to_string(pairindex)); // i wish to reference the actual name, it seems as if this version of YBSC is sorted by magnitude only.
                    starText.setCharacterSize(14);
